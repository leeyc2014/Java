...
public class CountTest { //file 이름과 같아야 한다
	...
	public static void main(String[] args) {
		Count.print1(); //클래스 호출
		//print1(); //CountTest 밖의 클래스에 있는 메소드(생략 불가능)
		Count.totalCount++;//클래스 필드의 사용 235페이지 (2)클래스 필드 사용
		//Count.count++;//오류 발생(count는 클래스 필드가 아니라 인스턴스 필드 - 인스턴스 생성 후 참조변수 필요)
		Count c = new Count();
		c.count = 10;
		showStatic(); //같은 클래스 내에 있으므로 "클래스."생략 가능
		...
		c1.count++; 
		c1.totalCount++;//클래스 필드를 접근하는 바람직한 방법 아님(객체의 참조변수에 사용X)
