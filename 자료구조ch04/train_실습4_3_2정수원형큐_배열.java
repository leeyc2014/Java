...
import 자료구조ch04.IntQueue3.EmptyIntQueue3Exception;
import 자료구조ch04.IntQueue3.OverflowIntQueue3Exception;

//int형 고정 길이 큐

class IntQueue3 {
	private int[] que; // 큐용 배열 - ArrayList로 구현 시 원형큐 유지 불가
...
//--- 큐에 데이터를 인큐 ---//
	public boolean enque(int x) throws OverflowIntQueue3Exception {
		if(front == rear && isEmptyTag == true) {
			throw new OverflowIntQueue3Exception("큐 enque 오버플로우");
		}
		que[rear++] = x;
		isEmptyTag = false;
		return true;
...
//--- 큐에서 데이터를 디큐 ---//
	public int deque() throws EmptyIntQueue3Exception {
		if(front == rear && isEmptyTag == false) {
			throw new EmptyIntQueue3Exception("큐 deque 언더플로우");
		}
