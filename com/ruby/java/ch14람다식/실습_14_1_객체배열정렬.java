...
public class 실습_14_1_객체배열정렬 {
	...
	static void printStudents(String msg, String rep, Student[] data) {
	...
	public static void main(String[] args) {
		...
		printStudents(">>>익명 객체를 이용해서 이름으로 오름차순 정렬", "-", data);

		// 람다함수를 이용해서 이름으로 내림차순 정렬
		Arrays.sort(data, (s1, s2)->s2.sname.compareTo(s1.sname));
		printStudents(">>>람다함수를 이용해서 이름으로 내림차순 정렬", "=", data);
		//==================================================================
		...
		printStudents(">>>익명 객체를 이용해서 번호로 오름차순 정렬", "-", data);

		// 람다함수를 이용해서 번호로 내림차순 정렬
		Arrays.sort(data, (s1, s2)->s2.sno-s1.sno);
		printStudents(">>>람다함수를 이용해서 번호로 내림차순 정렬", "=", data);
		//==================================================================
		// 익명 객체를 이용해서 생년으로 오름차순 정렬

		// 앞 코드를 참고해서 구현
		Comparator<Student> compBirth = new Comparator<>() {
			public int compare(Student s1, Student s2) {
				return (s1.sbirth.compareTo(s2.sbirth));
			}
		};
		Arrays.sort(data, compBirth);
		printStudents(">>>익명 객체를 이용해서 생년으로 오름차순 정렬", "-", data);
		// 람다함수를 이용해서 생년으로 내림차순 정렬

		// 앞 코드를 참고해서 구현
		Arrays.sort(data, (s1, s2) -> s2.sbirth.compareTo(s1.sbirth));
		printStudents(">>>람다함수를 이용해서 생년으로 내림차순 정렬", "=", data);
	}
}
