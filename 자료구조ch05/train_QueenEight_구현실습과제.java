...
class Stack5 {
	...
	public boolean push(Point x) throws OverflowGenericStackException {
		if (isFull()) {
			throw new OverflowGenericStackException("overflow stack exception");
		}
		data.add(x);
		top++;
		return true;
	...
	public Point pop() throws EmptyGenericStackException {
		if (isEmpty()) {
			throw new EmptyGenericStackException("empty stack exception");
		}
...
public class train_QueenEight_구현실습과제 {
	...
	public static void EightQueen(int[][] d) {
		...
		try {
			...
			System.out.println("총 해의 개수: " + numberOfSolutions);
		} 
		catch (Exception e) {
			e.printStackTrace();
		}
...
//배열 d에서 (x,y)에 퀸을 배치할 수 있는지  조사
	public static boolean checkMove(int[][] d, int x, int y) {// (x,y)로 이동 가능한지를 check
		if (checkDiagSE(d, x, y) == true || checkDiagSW(d, x, y) == true || checkCol(d, y) == true
				|| checkRow(d, x) == true) {
			return false;
		} else {
			return true;
