...
abstract class Item {
	...
	// 생성자
	public Item(String name, double price, int stokQuantity) {
		this.name = name;
		this.price = price;
...
class Order extends SeasonalDiscount {
	...
	public void printOrderSummary() {
		...
		for(int i = 0; i < count; i++) {
			...
			System.out.print(", 가격 : " + (items[i].getPrice() * quantities[i]));	
...
class RegularCustomer extends Customer {
	...
	@Override
	double getDiscountedPrice(double price) {
		return price - (price * REGULARDISCOUNT_RATE);
	}
}
...
class PremiumCustomer extends Customer {
	...
	@Override
	double getDiscountedPrice(double price) {
		return price - (price * PREMIUMDISCOUNT_RATE);
	}
}
